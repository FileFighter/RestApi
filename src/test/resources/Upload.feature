Feature: Upload Files
  ...
  Scenario: Successful interaction, upload of file in personal folder
    When the user with token "420" wants to upload a file with the name "unterkünfte.avi", path "unterkünfte.avi", mimeType "text/html" and size 123 to the folder with the id 42
  ...
  Scenario: Successful interaction, upload of file in shared folder
    ...
    Then response status code is 201
    And the response contains the item with path "/nasir/gebäude/eggs.food" and name "eggs.food" and mimeType "text/html" and type "TEXT" and size 123
    When user with token "9000000" wants to see the content of folder with path "/nasir/gebäude"
    Then the response contains the file with name "eggs.food"
    And the response contains the file with fileSystemId 72 and name "Bergfried.avi"
  ...
  Scenario: Successful interaction, merge file into existing folder
    When the user with token "420" wants to upload a file with the name "Vorratslager.avi", path "GeBäUdE/Vorratslager.avi", mimeType "text/html" and size 400 to the folder with the id 420
    Then response status code is 201
    When user with token "420" wants to see the content of folder with path "/Nasir/Gebäude/"
    Then response status code is 200
    And the response contains the file with name "Vorratslager.avi"
    And the response contains the file with name "Bergfried.avi"
  ...
  Scenario: Overwriting existing file.
    Given fileSystemItem with the fileSystemId 1234 is a folder and contains the fileSystemId 300
    And fileSystemItem with the fileSystemId 300 exists, has owner with userId 1234 and name "Sparta.csv"
    When the user with token "9000000" wants to upload a file with the name "Sparta.csv", path "Sparta.csv", mimeType "text/plain" and size 300 to the folder with the id 1234
    Then response status code is 201
    And the response contains the file with name "Sparta.csv" and size 300
    When user with token "9000000" wants to see the content of folder with path "/Richard/"
    Then response status code is 200
    And the response contains the file with name "Sparta.csv" and size 300
    When the user with token "9000000" wants to upload a file with the name "Sparta.csv", path "Sparta.csv", mimeType "text/plain" and size 299 to the folder with the id 1234
    Then response status code is 201
    And the response contains the file with name "Sparta.csv" and size 299
    When user with token "9000000" wants to see the content of folder with path "/Richard/"
    Then response status code is 200
    And the response contains the file with name "Sparta.csv" and size 299
  ...
  Scenario: Is not a folder
    When the user with token "420" wants to upload a file with the name "NaDasIstJaGesternNichtSoGutGelaufen.mp3", path "kiSounds/NaDasIstJaGesternNichtSoGutGelaufen.mp3", mimeType "audio/mpeg" and size 99989 to the folder with the id 72
    Then response status code is 400
