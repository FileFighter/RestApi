Feature:
  As a user
  I want to be find another user
  ...
  Scenario: Successful find another user
    ...
    And response contains key "username" and value "penguin"
    And response contains key "id" and value 1235
  ...
  Scenario: Failed to find another user because username has spelling errors
    ...
    And response contains key "message" and value "benguin does not exist"
  ...
  Scenario: Failed to find another user because username does not exist
    ...
    Then response status code is 404
    And response contains key "message" and value "bielefeld does not exist"